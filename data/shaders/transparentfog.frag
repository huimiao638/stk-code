...
void main()
{
    ...
    diffusecolor.rgb =
        0.2 * (DiffuseIBL(normal, eyedir, 0.1, diffusecolor.rgb) + SpecularIBL(normal, eyedir, 0.1, vec3(0.04))) +
        sun_col * NdotL * (DiffuseBRDF(normal, eyedir, Lightdir, diffusecolor.rgb, 0.1) + SpecularBRDF(normal, eyedir, Lightdir, vec3(.04), 0.1));
